...
md`# RAMPVIS Idiom: Ontology-Assisted Visualization`
)});
...
In our specific context, the user is the RAMPVIS system *infrastructure developer and manager* (simply *manager* in the following) who is a volunteer developer. They manage the visualization infrastructure, its data streams, user interface, and interactive data visualizations. At any point in time, the system involves hundreds of data sets and dozens of visualizations. A visualization can be anything, e.g., from a static barchart to an interactive visualization of geotemporal or relational data (see Section 2).
...
## 3. NAME - and description`
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`
### Ontology-assisted visualization

Gilson et al. [Gilson 2008] suggested using ontologies for the automatic generation of visualization plots in a doctoral project. An ontology is a formal represenation of knowledge that is widely used, e.g., in Biology but also in Engineering and library science to formally describe resources, their attributes, and relationships. It can best be thought of as a huge knowledge graph. Khan et al. [Khan 2016] extended the use of ontology for provenance visualization in an industrial doctoral project. In RAMPVIS, we transformed the ideas in these two doctoral projects into a deployable VIS infrastructure.
...
## 4. ORIGIN & CONTEXT`
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`
Carpendale et al. [2014] reflected on the use of ontology-based visualization in biology, as a starting point for thinking about how ontologies could be used more broadly in visualization. Among their suggestions was the use of ontologies for visualization generation, with the structured relationships in the ontology helping to simplify the visual design. Gilson et al. [2008] described a visualization approach that combined ontology mapping with probabilistic reasoning to produce new visualizations - this was exemplified by an implementation that automatically visualized music chart data. Yu and Silva [2016] used data flow diagrams to support visualization; whilst not an ontology, this also provides evidence that structured data representations can help generate new visualizations.
`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`___
...
FileAttachment("ontology-deatiled-example.png").image()
)});
  main.variable(observer()).define(["md"], function(md){return(
md`
...
2. When a new visualization is needed for a data stream, the infrastructure manager either choses an existiging visualization from our pool of visualizations or asks a separate visualization developer to create a new visualization, e.g., using D3.js. The infrastructure manager then describes the binding of the data set to a visualization: *P(V,D)*. This binding establishes a relationship between the relevant data streams and the visualization function, so that streams are passed as function arguments in the correct order. This results in an appropriate and valid visualization for the data, accessible through a web page. 
3. Finally, the infrastructure manger can use this ontolgoy to *propagate* this binding (i.e., the mapping between data set and visualization) to other similar data sets. To that end, the manager uses a search UI to find suitable data streams then performs quality assurance on search results. When a decision is made to propagate a visualization function to data stream(s), a propagation service operates using the ontology. Propagated visualizations are immediately published as interactive web pages. The supplementary video walks through this workflow [Khan 2022]. More information on how this propagation works can be found in our other notebook: https://observablehq.com/d/87a416cd4468fff0
...
md`### 7.2 What Didn't Work`
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`
Implementation of ontology is a challenge. We developed our first version of Ontology using plain JSON declarative syntax. We had 5-6 VIS functions, less than 100 data sets, and we created around ~100+ pages. But when the dataset, VIS function, and requirement for more pages grew, we had to improve the implementation. Ontology in a scalable database and write complex business logic for Ontology operations. We are also able to implement advanced search and propagation logic on top of that.

We implemented a simple and quick version of ontology, it worked well for genering ~100 pages. Later on we improved the implementation to scale and genearted ~850 plots. This solution is now scalable enough to semi-automatically generate many more plots.
`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`### 7.3 Evidence`
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`
We have been able to deploy 850 plots and dashboards from 17 reference visualization function implementations. The ontology supported the rapid deployment of these plots and dashboards through a search-and-propagate process, with no additional development effort from the visualization designer.
...
## 8. REFERENCES`
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`
Gilson, O. and Silva, N. and Grant, P. W. and Chen, M. (2008).<br/>
From Web data to visualization via ontology mapping.<br/>
...
Ontology-assisted Provenance Visualization for Supporting Enterprise Search of Engineering and Business Files.<br/>
...
Ontologies in Biological Data Visualization.<br/>
IEEE Computer Graphics and Applications, vol. 34, no. 2, 8-15.<br/>
https://doi.org/10.1109/MCG.2014.33

Yu, B., and Silva, C. T. (2016).<br/>
VisFlow - Web-based Visualization Framework for Tabular Data with a Subset Flow Model.<br/>
