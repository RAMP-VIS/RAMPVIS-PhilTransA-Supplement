...
Advisory Group, and the use of metaphors to explain *modelling*. Early infographics used data from the Modelling Team Supporting 1-km2 Spatial Simulation. The use of some pictograms is anticipated in the data-driven interviews prepared by the Disseminative Vis Team.`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`---
...
## 3. NAME - and description`
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`In our context, *pictograms* are visualisations that use stylized figurative drawings (icons, images etc.) to represent data ([Borgo et al.](http://diglib.eg.org/handle/10.2312/conf.EG2013.stars.039-063)), often in a space-saving and language-saving manner ([Davies et al., 1998](https://www.sciencedirect.com/science/article/pii/S0003687097000215?via%3Dihub)).`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`---
## 4. ORIGIN & CONTEXT`
...
These infographics were used to generate video sequences (one slide per frame) to present key messages. The pictograms were used in many of the slides to depict key message elements, such as a lockdown indicated through the use of a lock icon in a geographical pin icon, or populations of symptomatic/un-symptomatic and non-infected individuals, such as with stick figures of different colours. Likewise, social distance was depicted with a circle around a stick figure. The creation of this ideas was not systematic, and as a consequence the icons generated are not consistent, or part of a larger family of icons/glyphs/pictograms. `
)});
  main.variable(observer()).define(["FileAttachment","htl"], async function(FileAttachment,htl){return(
htl.html`<img src="${await FileAttachment("2-blank.png").url()}" style="height: 300px"/>`
)});
  main.variable(observer()).define(["FileAttachment","htl"], async function(FileAttachment,htl){return(
htl.html`<img src="${await FileAttachment("6-blank.png").url()}" style="height: 300px"/>`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`---
## 7. REFLECTION`
...
md`### 7.1 What Worked`
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`Pictograms are an easy way to create succinct, impactful messaging, which follows the mantra "an image tells a thousand words". Especially in a world where the imaging relating to the pandemic has become frequent, it is easy to convey messages this manner. The technique is popular in a number of messages from governments and health organisations (see Section 5), whereas their efficiency in broad terms is indicated by their adoption in messaging from major organisations (see Section 6, e.g., WHO, CDC etc). In terms of their efficiency in data visualisation applications, works, such as from [Borgo et al., 2012](https://doi.org/10.1109/TVCG.2012.197), indicate they can aid memorization, visual search and concept comprehension. `
)});
  main.variable(observer()).define(["md"], function(md){return(
