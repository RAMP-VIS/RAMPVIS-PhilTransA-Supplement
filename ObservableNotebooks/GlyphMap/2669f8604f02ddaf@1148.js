...
## 3. Gridded Glyphmaps`
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`**Gridded Glyphmaps** aggregate dense spatial data into grid cells, creating a tessellating set of square "tiles". Within each tile, a subplot/glyph (miniature information graphic) summarises the multivariate data for the ground-area represented by the tile. A wide variety of subplot/glyph designs are possible and can be designed to depict a wide range of multivariate data, including indicate counts/averages by category, statistical distributions, time-series and various types of comparison.`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`The gridding is done in screen space. This means that in interactive versions, the act of "zooming" changes the level of aggregation and the subplots/glyphs are re-rendered accordingly.`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`---
## 4. ORIGIN & CONTEXT`
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`Placing glyphs/subplots on maps has a long history. Subplots (miniature charts) such as barcharts, piecharts, timeseries graphs are often placed on choropleth maps to depict multivariate data about them, including [Jacque Bertin in his influential book](https://www.semanticscholar.org/paper/Semiology-of-Graphics-Diagrams%2C-Networks%2C-Maps-Bertin/22be1cebc94d9eedc2cc6c000bee5222a37a9fd0).
Icons or glyphs for representing multiclass and multivariate fields (data that continuouly vary data over space) have been demonstated by [Pickett and Grinstein](http://www.ifs.tuwien.ac.at/~silvia/wien/vu-infovis/references/pickett-ieee88.pdf) and more recently by [Wickham *et al* (2012)](https://vita.had.co.nz/papers/glyph-maps.html). Wickham demonstrates "glyphmaps" - how arranging subplots/glyphs in regular arrays is an effective method for depicting environmental data in the context of climate science. He compared them to heatmaps in which multivariate glyphs replace colour lightness indicating a univariate value. There was a focus on gridded data but it wasn't limited to that.

[Slingsby, 2016](https://openaccess.city.ac.uk/id/eprint/20884/) developed some of the ideas further including the use of interactivity for exploring data at multiple spatial spaces. The term "Tilemaps" was used in this work, but was not limited these to gridded aggregations, but also appplie to aggregations that are based on irregular regions (e.g. administrative units) that are allocated into cells in a regular array using algorithms (e.g. [Meulemans *et al*, 2016](http://openaccess.city.ac.uk/15167/)).`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`___
...
## 5. EXAMPLES`
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`Bertin (1987) has some examples of multivariate map data for France (e.g. [example here](http://philogb.github.io/blog/2013/06/09/france-mobility-flow/). There are also plenty of examples in [Wickham *et al* (2012)](https://vita.had.co.nz/papers/glyph-maps.html) (below). Many of them depict timeseries data of climate-related variables, with the one on the left depicting smoothed trends of temperature prediction using locally-scaled glyphs and globally-scaled cell-shading. Examples are in the paper (I haven't obtained permission to use them here).

Multiscale species accumulation curves ([Morgades et al, 2021](https://openaccess.city.ac.uk/id/eprint/26289/)) use a similar technique to show median species richness (y-axis) at different spatial scales (x-axis), with the red band indicating the interquartile range, which converges to zero to the right as the scale reaches the scale represented by the grid cell.`
)});
  main.variable(observer()).define(["FileAttachment"], async function(FileAttachment){return(
await FileAttachment("pic4.png").image({width:600})
)});
  main.variable(observer()).define(["md"], function(md){return(
md`Some population-based glyphs in [Slingsby *et al* (2016)](https://openaccess.city.ac.uk/id/eprint/20884/) represent population profiles (as a radial "spider plot") from the 2010 UK census with variable types distinguished using hue. The variable values are actually comparisons to the mean, with circular glyphs near the centre of Leicestershire (where most of the population resides) indicating a profile that is close to the average for the area and outlying areas (where fewer people live) deviating from this average "circle". They enable us to pick out spatial foci of particular socio-economic groups and see broad trends, such as the differences between the city and its suburbs and the eastern and western neighbourhoods.
`
)});
  main.variable(observer()).define(["FileAttachment"], async function(FileAttachment){return(
await FileAttachment("Screenshot 2021-08-18 at 14.07.29.png").image({width:600})
)});
  main.variable(observer()).define(["md"], function(md){return(
md`Some temporal glyphs are used in [Slingsby and van Loon (2017)](https://openaccess.city.ac.uk/id/eprint/18270/) to show data that were collected from GPS-tracked gulls in a study of the movement of several animals in the Netherlands.
Here, the columns (x-axis) in each glyph are time of day and the rows (y-axis) are month of year. So the glyph is a compact calendar that can show daily activity varying over time. Colour lightness indicates time-spent by gulls in each map cell.
This indicates at which times in the year and day gulls are present.`
)});
  main.variable(observer()).define(["FileAttachment"], async function(FileAttachment){return(
await FileAttachment("Screenshot 2021-08-18 at 14.10.58.png").image({width:600})
)});
  main.variable(observer()).define(["md"], function(md){return(
md`---
## 6. DESCRIPTION & USE`
...
It is useful to be able to verify **model inputs** as this drives model outputs.
These screenshots show (inverted - older people are at the bottom) population pyramids that represent population counts in age groups:
**absolute numbers** (left), **relative numbers**, to show age structure (middle) and **relative numbers with _opacity_ based on absolute numbers** (right). The latter de-emphases places where population is small, the pyramics are more transparent where fewer people reside. 
...
<img src="${await FileAttachment(
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`Below is a good example of the value of the relative population gylphs. It shows that Central Edinburgh and Glasgow have unusually many younger people than elsewhere. This is a characteristic of many inner-cities.`
)});
  main.variable(observer()).define(["FileAttachment"], async function(FileAttachment){return(
await FileAttachment(
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`These Gridded Glyphmaps are interactive and can be explored at any spatial scale (see examples below)

There are also **intermediate internal parameters**. Inspection of these can help ensure that the model is performing as expected. The animation shown below is for the the intermediate and internal "force of infection" parameter.
These are time series glyphs, with time running from left to right.
  Note the double-peak in some areas.`
)});
  main.variable(observer()).define(["FileAttachment"], async function(FileAttachment){return(
await FileAttachment("Aug-05-2021 17-06-00.gif").image({width:600})
)});
  main.variable(observer()).define(["FileAttachment","md"], async function(FileAttachment,md){return(
md`### See how the multivariate model outputs change spatially over time

This requirement is rather exploratory and is designed to help assess whether the model is doing what is expected. As the model is spatially-explicit over time, this animated mode of visualization supports this task well. The screenshot below shows numbers with different modelled infection statuses over time (left-right; blue is "susceptible", i.e. people who have not been infected).
As above the maps show: **absolute numbers** (left), **relative numbers** (middle) and **relative numbers with opacity based on absolute numbers** (right; which de-emphases places where population is small).
...
<img src="${await FileAttachment(
  ...
  main.variable(observer()).define(["FileAttachment","md"], async function(FileAttachment,md){return(
md`This helps us **assess the effect of the infectiousness parameters on the modelled spread**. Below is an example of a model run in which infection does not spread beyond densely populated areas. Relative coloured areas indicate the proportion of population in different infection modes; the one on the right has the "susceptible" (i.e. not infected) population removed. It is clear that in this case **the parameter for infectiousness is too low to model outside populated areas**.

<img src="${await FileAttachment(
  "pic5.png"
).url()}" width=300/><img src="${await FileAttachment(
  "pic6.png"
).url()}" width=300/>`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`The descriptions and animated images below show how the Gridded Glyphmap idiom can help determine this.

Below is an animation that demonstrates zooming for **changing the spatial resolution**. It shows absolute numbers of people affected by COVID. The unsurprising **key observation** is that this is all in the heavily populated "Central Belt". The animation shows how COVID infection spreads over time (see top for day number).`
)});
  main.variable(observer()).define(["FileAttachment"], async function(FileAttachment){return(
await FileAttachment("Aug-05-2021 15-41-16.gif").image({width:600})
)});
  main.variable(observer()).define(["md"], function(md){return(
md`Below is a video that shows **relative proportion of population** and clearly shows the progression of COVID infection over space (blue is "susceptible" which represented the share of population that has not been infected). The **key observation** is the spatial coherence in spread that lags spatially. It demonstrates the spatial pattern of temporal lags that result from the variety infection "seed" locations and the interactions between these.

Various interactions are available in the software, including:
 ...
 - Move to a different area (left-drag mouse; pan)`
  ...
  main.variable(observer()).define(["FileAttachment"], async function(FileAttachment){return(
await FileAttachment("Aug-05-2021 15-16-21.gif").image({width:600})
)});
  main.variable(observer()).define(["md"], function(md){return(
md`### See comparisons of model outputs with different input parameters

Where we have two different model runs, we look at them one after the other or next to each other to help us compare.
However, it is more effective to **compute the differences** and show these differences explicitly with graphical encodings.
In this section, we compare each infection category between two model outputs, computing the differences.

Bars above the horizontal centreline indicate **more**; bars below the horizontal centreline indicate **less**; 

The graphic below shows **absolute difference** (in numbers of people). Below the centreline is where the difference is negative (less than); above the centreline is where the different is positive (more than). Time is shown from left to right. No difference is observed for the first part of the period, and then the graphic clearly shows that more people are affected in *abundances_poll1.h5* than *abundances_poll2.h5* in the latter half of the time periods.

The tooltip indicates a situation where there are *fewer* people in the susceptible disease state (because they are infected) and also provides percentages.`
)});
  main.variable(observer()).define(["FileAttachment"], async function(FileAttachment){return(
await FileAttachment(
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`Below we see the same graphic, but this version shows differences as a **proportion of whole population**.
  
Note that some of the highest proportions are where there is low population. The design challenge is to develop glyphs that show both relative and absolute numbers concurrently in ways that are interpretable. The tooltip indicates 11% more exposed on day 20 than in *abundances_poll2.h5*.
`
)});
  main.variable(observer()).define(["FileAttachment"], async function(FileAttachment){return(
await FileAttachment(
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`A solution to showing both absolute and relative difference is shown below.
  
In this design, grid cells with low population are deemphasised (faded). The tooltip shows 9% more people susceptible (because they are not infected), but the lightness indicates a comparatively low population.
  
Note the flipping of the "exposed" category (yellow) from positive to negative to to the right of the tooltip - interesting!`
)});
  main.variable(observer()).define(["FileAttachment"], async function(FileAttachment){return(
await FileAttachment(
...
You can interact with this to see how the Gridded Glyphmaps work at multiple resolutions by dragging and clicking the map to _pan_ and _zoom_, and using the slider to _change grid resolution_. `
)});
