...
md`# Represent error and uncertainty`
)});
...
## 3. NAME - and description`
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`We aim to **represent error and uncertainty** from multiple configurations through multiple linked views and interactions. There are **aleatoric/statistical uncertainty**, **structural uncertainty** and **prediction uncertainty** from the output ([Sedlmair et al. (2014)](https://doi.org/10.1109/TVCG.2014.2346321)).`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`---
## 4. ORIGIN & CONTEXT`
...
[Fua et al. (1999)](http://ieeexplore.ieee.org/document/809866/) use a hierarchical parallel coordinates combined with colors and transparency to depict uncertainty in a fatal accident dataset. Extent scaling, a technique to uniformly scale overlapping bands to reveal relative sizes of the extents, is applied to reduce occlusions. See figure below, left (without extent scaling) vs. right (with extent scaling).

${await FileAttachment("fua1999.png").image()}
...
![](https://onlinelibrary.wiley.com/cms/asset/83ec8664-2915-4846-864c-3a9dce28328c/mfig002.jpg)`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`---
## 6. DESCRIPTION & USE`
...
${await FileAttachment("ensemble_line_chart@2.png").image()}

Other abnormal outputs (eg. flat curves) were also observed during our demonstration with the EMBs.

${await FileAttachment("line_2.png").image()}
${await FileAttachment("line_3.png").image()}


In some outputs, uncertainty is encoded with confidence bands.
${await FileAttachment("line_ci.png").image()}
