...
* I would like to know
  * if there is any error in the simulation module
  * how the parameters affect the output
  * the effect of different isolation policy options
  * develop an intuitive feel/understanding of the dynamics 
* so that I can 
  *  detect and fix the error in the simulation code
  *  choose the right parameter values that lead to output that matches to the reality
  *  understand the balance between the lost of freedom and the size of the infected population for different isolation policies.
  *  guide the quantitative measures we extract from the simulations
  *  helps conceptualise control measures that tackle key features of the dynamics`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`---
...
## 2. DATA`
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`The data used for the GMap visualisation is generated by the simulation model (written in Java), which is available on Github https://github.com/ScottishCovidResponse/Contact-Tracing-Model. The dataset used to generate the GMap visualisations below are available on GitHub: https://github.com/jeewanhyongju/InfectionMap-Visualization`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`---
...
## 3. NAME - and description`
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`Traditional graph visualization methods often fail to capture the underlying structural information, clustering, and neighbourhoods. **GMap** provides a way to overcome some of these shortcomings with the help of the geographic map metaphor: nodes are grouped with coloured background, similar to regions on a geographical map.  GMap has the added advantage of familiarity comparing to other graph visualisation methods, as most people are very familiar with maps and even enjoy carefully examining them.`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`---
## 4. ORIGIN & CONTEXT`
...
The image below shows part of the GMap where the colour indicates the location of where a person is infected (such as school or shop). Each node is a person, and two are connected if one infects the other (the infection is directional but this is not shown in the visualisation). The person with a red number and a circle is the 'source' of each infection chain or cluster. Here the node background colour shows the location where the person is infected. Two people have the same background colour if they are infected at the same (type of) place. All the neighbouring persons with the same location form a cluster. There are 10 different types of locations including restaurant, school, shop, office, etc.

![gmap-location-as-cluster-crop.png](${await FileAttachment("gmap-location-as-cluster-crop.png").url()})

The other alternative is to use the infection chain as the cluster, i.e., all the people in the same infection chain has the same background colour. In the image below, the node shaped is used to show the location information as discussed earlier (circle is restaurant, square is school, etc.)

![ConnectedNodeAsCluster_50_zoom_in.png](${await FileAttachment("ConnectedNodeAsCluster_50_zoom_in.png").url()})

The size of the clusters gives the users some idea of how many people are infected and how fast the spread is (a larger cluster usually means a faster spread). The epidemiological modellers can mentally compare the visualisation with what they expected to identify any obvious issues in the model. Comparing visualisations from different isolation policies allow the users to see the difference in policy impact and their dynamics.`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`---
## 7. REFLECTION`
...
![ConnectedNodeAsCluster_50_1000px.png](${await FileAttachment("ConnectedNodeAsCluster_50_1000px.png").url()})

Another challenge is that the computation of the GMap layout can be quite slow, especially for large graphs. This will make it not suitable for interactive visualisation, even if a JavaScript version existed.`
)});
  main.variable(observer()).define(["md"], function(md){return(
...
`
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`Xu, Kai and Turkay, Cagatay and Mohr, Sibylle and Matthews, Louise and Archambault, Daniel`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`---
...
## 8. REFERENCES`
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`Gansner, E.R., Hu, Y. and Kobourov, S. (2010) ‘GMap: Visualizing graphs and clusters as maps’, in 2010 IEEE Pacific Visualization Symposium (PacificVis). 2010 IEEE Pacific Visualization Symposium (PacificVis), pp. 201–208. doi:10.1109/PACIFICVIS.2010.5429590.

Jianu, R. et al. (2014) ‘How to Display Group Information on Node-Link Diagrams: An Evaluation’, IEEE Transactions on Visualization and Computer Graphics, 20(11), pp. 1530–1541. doi: 10.1109/TVCG.2014.2315995.
`
)});
