...
md`# RAMPVIS Idiom: Propagating Visual Designs`
)});
...
*Note: This section is identical between idioms proposed by generic vis team.*
`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`---
...
## 3. NAME - and description`
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`
### Propagating Visual Designs

Mackinlay outlined the need for and challenge of automatic generation of visualization (Mackinlay 1986 ToG). We have developed a new approach to achieving this using automated propagation of existing visual designs to similar data (Saiful 2022 TVCG). Our novel approach uses an ontology to facilitate propagation, which provides flexibility and generalization - i.e., this approach is not inherently linked to the semantics of our data.
`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`---
...
FileAttachment("propagation.png").image()
)});
  main.variable(observer()).define(["md"], function(md){return(
md`
...
3. A quality assurance step carried out by an expert ensures the visual design is only propagated to suitable data, resulting in **new instances of existing visualizations** applied to the selected data streams, that are immediately deployed as new web pages [Khan 2022].
`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`---
...
md`
Mackinlay, J. (1986).<br/>
Automating the Design of Graphical Presentations of Relational Information.<br/>
...
Show Me: Automatic Presentation for Visual Analysis.<br/>
...
A Survey on Automatic Infographics and Visualization Recommendations.<br/>
