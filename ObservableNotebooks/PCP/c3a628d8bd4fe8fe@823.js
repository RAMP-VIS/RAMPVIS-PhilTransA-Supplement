...
md`# RAMPVIS Idiom : Parallel Coordinates Plots`
)});
...
## 3. NAME - and description`
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`Parallel Coordinate Plots (PCPs) introduced by Inselberg ([Inselberg et al.(1990)](https://ieeexplore.ieee.org/document/146402)) is one of the most useful techniques for displaying large multivariate datasets. PCPs provide a graphical representation of relationships between data variables by connecting polylines that represent a data record, through the parallel axes representing variables and their ranges. In practice, a PCP design can display high-dimensional data with up to 10-15 dimensions, as each axis is visually separated ([Kosora et al.(2006)](https://ieeexplore.ieee.org/abstract/document/1634321)). PCPs allow us to present parameter space and variations. Implementation of the interaction provides the parameter space analysis and comprehension of complex layers in the multivariate data. The use of brushing and interaction enables users to reveal relationships and tendencies between both adjacent and non-adjacent axes.  
`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`---
## 4. ORIGIN & CONTEXT`
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`Parallel Coordinate Plot (PCP) introduced by ([Inselberg et al.(1990)](https://ieeexplore.ieee.org/document/146402)) is a visual design showing multidimensional relations by using parallel axes. Surveys on PCP design focus on evaluation and categorization of PCP studies ([Johansson et al. (2015)](https://ieeexplore.ieee.org/abstract/document/7192677?casa_token=UPXK5vRXQq0AAAAA:3JU9blB00XcL7byMiewoG5H-g0PUY_wgasxYNz-TonjZWx8zrkfviMbRSYpnywNe55WmhA)) and cover geometric models and reviews methods for constructing and understanding PCPs ([Heinrich et al.(2013)](https://diglib.eg.org/handle/10.2312/conf.EG2013.stars.095-116)). Another survey by ([Dasgupta et al.(2012)](https://www.semanticscholar.org/paper/Conceptualizing-Visual-Uncertainty-in-Parallel-Dasgupta-Chen/44cb1910be9b2362b2b6bb757f4eeab62546c177))  searches for several sorts of ambiguity in parallel coordinate images and proposes a taxonomy for classifying them in order to reduce uncertainty. The goal was to detect different sources of uncertainty in the design and link them to different implications of uncertainty on the user by building a taxonomy.
`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`___
...
md`Siirtola et al. present an example parallel coordinate plot of the car data set which helps to understand the relationships between the different attributes of a car ([Siirtola et al.(2006)](https://academic.oup.com/iwc/article-abstract/18/6/1278/682231?redirectedFrom=fulltext)). The PCP shows 9 attributes in the car dataset with their ranges (e.g. Cylinders min-max values [3-8]). Each edge represents a car entry that crosses each parallel axis to show the value on the axis for each entry. 
![pcpwithcardata@1.PNG](${await FileAttachment("pcpwithcardata@1.PNG").url()})

PCP allows us to see the expected strong inverse relationships between weight and acceleration (ACCEL), and economy (MPG), and the number of cylinders (CYL). In order to find more, we need to highlight cars of a particular year or origin to see how they vary across multiple variable axes. PCPs also show 'signatures' across multiple variables but are highly dependent upon axis ordering. In this example, it is challenging to relate the number of cylinders (CYL) with acceleration here, but it would be straightforward to see the correlation if they were adjacent axes. Once again, this highlights the need for interaction. Displaying large datasets results in a cluttered PCP view by overlapping the edges. This challenge can be aided by the brushing feature which is based on selecting a certain area on an axis. This will update the PCP view and show only the selected edges across the axes on the PCP. For example, the work by (Roberts et al.(2018)) introduces smart brushing techniques that improve the standard interactive brushing of a PCP.
...
>![ensemble_pcp_1@1.png](${await FileAttachment("ensemble_pcp_1@1.png").url()})
...
Evaluation of parallel coordinates: Overview, categorization and guidelines for future research._<br/>
