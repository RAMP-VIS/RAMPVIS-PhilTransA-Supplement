...
md`# Multiple Linked Views for Visual Exploration or how to Project for 2D - part 1`
)});
...
## 3. NAME - and description`
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`**Multiple Linked Views (MLV)** or **multiple coordinated views** refers to *any instance where data is represented in multiple windows*. Different representations of data are placed in multiple windows where interactions on these views are coordinated ([Robert (2007)](https://ieeexplore.ieee.org/document/4269947)). By adopting MLV, our implementation enables users to **rapidly explore the complex simulation data**.([North and Shneiderman(2000)](https://dl.acm.org/doi/10.1145/345513.345282)).`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`---
## 4. ORIGIN & CONTEXT`
...
md`**Improvise** ([Weaver (2004)](https://ieeexplore.ieee.org/document/1382904)) uses MLV to control various aspects of the visualization coordinately.

${await FileAttachment("commongis.png").image()}

**[Hadlak et al. (2011)](https://ieeexplore.ieee.org/document/6065000)** uses MLV to enable exploration of the selected data dimension.

${await FileAttachment("situ.png").image()}`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`---
## 6. DESCRIPTION & USE`
...
${await FileAttachment("ensemblevis_mlv.png").image()}
...
${await FileAttachment("MLV-3.png").image()}

And with this particular simulation configuration, there is an unusual pattern around day 21 in the output (see the figure below). **Similar patterns were observed with several other configurations, which prompted an investigation by the EMBs.**

${await FileAttachment("ensemble_line_chart.png").image()}


EMBs are generally familiar with visual designs such as PCP and heatmap, they were surprised by the amount of information made available by combining them.

Other than a screen sharing session from the developer during the meetings, the system is hosted live (in the form of a web application), this enables the EMBs to explore the system in real-time. **We believe this fostered an excellent cross-disciplinary collaboration where we focus on a common objective.**
...
md`### 7.2 What Didn't Work`
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`The data size is huge (256,000,000 data points), the data processing is heavy. And given the time we had, the performance is not optimized and the projection of data may be improved with dimensionality reduction (which we briefly experimented with). **We are currently working together with the generic team on the optimization for data processing and storage.**

Our MLVs at the moment use a balanced representation strategy ([Hadlak et al. (2015)](https://diglib.eg.org/handle/10.2312/eurovisstar.20151109.001-020)) where each view is assigned the same amount of space, regardless of the amount of data the visualization is representing. The approach worked fine at the beginning, but we quickly realized the weakness as more and more new views were being introduced. **The strategy isn't the best choice as certain views should take priority over others, and we are currently working on allocating the limited screen space to the MLVs.**
...
## 8. REFERENCES`
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`
**Hadlak et al. (2011)** Hadlak, S., Schulz, H.-J., & Schumann, H. (2011). In Situ Exploration of Large Dynamic Networks. IEEE Transactions on Visualization and Computer Graphics, 17(12), 2334–2343. https://doi.org/10.1109/TVCG.2011.213

**Hadlak et al. (2015)** Hadlak, S., Schumann, H., & Schulz, H.-J. (2015). A Survey of Multi-faceted Graph Visualization. Eurographics Conference on Visualization (EuroVis) - STARs, 20 pages. https://doi.org/10.2312/EUROVISSTAR.20151109

**Roberts (2017)** Roberts, J. C. (2007). State of the Art: Coordinated amp; Multiple Views in Exploratory Visualization. Fifth International Conference on Coordinated and Multiple Views in Exploratory Visualization (CMV 2007), 61–71. https://doi.org/10.1109/CMV.2007.20

**Weaver (2004)** Weaver, C. (2004). Building Highly-Coordinated Visualizations in Improvise. IEEE Symposium on Information Visualization, 159–166. https://doi.org/10.1109/INFVIS.2004.12
`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`---
